	...
	ajax_processer: function (type, result)
	{
		...
		else
		{
			if (type == 'comments_form')
			{
				...
				var count=$('.pl[onclick*='+result.rsm['item_id']+']').find('.count').text();
				$('.pl[onclick*='+result.rsm['item_id']+']').find('.count').text(count*1+1);
...
AWS.User = 
{
	...
	invite_user: function(selector, img)
	{
	    ...
	    }, function (result)
	    {
	    	if (result.errno != -1)
	    	{
				selector.addClass('cur').attr('onclick','AWS.User.disinvite_user($(this))').text('邀请');
			}
	    	else if (result.errno == -1)
	        {
	            alert(result.err);
	...
	disinvite_user: function (selector)
	{
	    $.get(G_BASE_URL + '/question/ajax/cancel_question_invite/question_id-' + QUESTION_ID + "__recipients_uid-" + selector.attr('data-id'), function (result)
		{
			if (result.errno != -1)
	        {
	            ...
	            selector.removeClass('cur').attr('onclick','AWS.User.invite_user($(this))').text('邀请');
					...
					if (val.length >= 2)
					{
						switch (type)
						{
							...
							case 'topic' :
								...
								topicva=$.get(G_BASE_URL + '/search/ajax/search/?type=topics&q=' + encodeURIComponent($(element).val()) + '&limit=10',function(result)
								{
									if (result.length > 0)
									{
										ul.html('');
	...
	init_topic_edit_box: function (selector, type)
	{
		$(selector).click(function()
		{
			...
			$.each($(selector).parents('.aw-topic-bar').find('.topic-tag'), function (i, e)
			{
				if (!$(e).has('i')[0])
				{
					$(e).append('<i onclick="detetag()" class="icon iconfont close icon-delete"></i>');
					$(e).find('i').show();
				}
			});
				...
				switch (data_type)
				{
					...
					case 'question' :
						...
						$.post(G_BASE_URL + '/topic/ajax/save_topic_relation/', 'type=question&item_id=' + data_id + '&topic_title=' + encodeURIComponent($(this).parents('.aw-topic-bar').find('.topic-text').val()), function(result)
						{
							if (result.errno == 1)
							{
								_this.parents('.aw-topic-bar').find('.tag-bar').prepend('<span class="topic-tag lab fl" data-id="'+ result.rsm.topic_id +'"><a>' + _this.parents('.aw-topic-bar').find('.topic-text').val() + '</a><i class="icon iconfont close icon-delete" style="display: inline"></i></span>');
